defmodule SymphonyElixir.ExtensionsTest do
  ...
  test "github adapter delegates tracker operations" do
    ...
    assert {:ok, ["codex:ready"]} = GitHubAdapter.fetch_issues_by_states(["codex:ready"])
    assert_receive {:github_fetch_issues_by_states_called, ["codex:ready"]}

    assert {:ok, ["123"]} = GitHubAdapter.fetch_issue_states_by_ids(["123"])
    assert_receive {:github_fetch_issue_states_by_ids_called, ["123"]}

    assert :ok = GitHubAdapter.create_comment("123", "hello")
    assert_receive {:github_create_comment_called, "123", "hello"}

    assert :ok = GitHubAdapter.update_issue_state("123", "codex:done")
    assert_receive {:github_update_issue_state_called, "123", "codex:done"}
  end
